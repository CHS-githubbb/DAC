...
/*
Lenet5.cpp shows how to use the api in Layers.h
there are certain things to remind you:

1. this is not a very typical Lenet5 model, so you might find some
   some layers are missing, are some layers have different sizes;


2. the weight I used here is grabbed from a pytorch model WITHOUT
   ANY TRAINING, so the result is unsatisfactory, but is the same
   with that of the pytorch model;


3. you can not run this file on vivado hls without any modification
   I have modified some features that are only available using c++11
   (and also some standard fuctions including memcpy and memset are
   not available), but there is still some thing that you need to 
   modify, including how to link each file together(I think the linkage
   on vivado hls is a bit different from what we used to do);
*/
...
int main() {
	...
	Conv<dtype, 6, 1, 1, 1, 28, 28, 0, 1> conv1(features_0_weight, features_0_bias);

	void (*max_pool2)(dtype [1][6][28][28], dtype[1][6][14][14])
			= MaxPool<dtype, 2, 1, 6, 28, 28, 2>;

	void (*relu_s2)(dtype [1][6][14][14]) = ReLU<dtype, 1, 6, 14, 14>;

	Conv<dtype, 16, 5, 1, 6, 14, 14, 0, 1> conv3(features_3_weight, features_3_bias);

	void(*max_pool4)(dtype[1][16][10][10], dtype[1][16][5][5])
		= MaxPool<dtype, 2, 1, 16, 10, 10, 2>;

	void (*relu_s4)(dtype [1][16][5][5]) = ReLU<dtype, 1, 16, 5, 5>;

	FullConnect<dtype, 120, 1, 16, 5, 5> fc5(classifier_0_weight, classifier_0_bias);

	void (*relu_s5)(dtype [1][120][1][1]) = ReLU<dtype, 1, 120, 1, 1>;

	FullConnect<dtype, 84, 1, 120, 1, 1> fc6(classifier_2_weight, classifier_2_bias);

	FullConnect<dtype, 10, 1, 84, 1, 1> fc7(classifier_3_weight, classifier_3_bias);
